...
void* initalize_nvm_space(const char *workspace_path, uint64_t max_num_chunks) {
    ...
    if ((chunk_region_start = mmap(0x7f0000000000, max_chunks*CHUNK_SIZE, PROT_NONE, MAP_SHARED|MAP_ANONYMOUS|MAP_NORESERVE, -1, 0)) == MAP_FAILED) {
        error_and_exit("Unable to mmap initial block of %lu chunks\n", max_chunks);
    }

    base_path_length = strlen(workspace_path);
    backing_file_path = (char*) sbrk(base_path_length + 1 + 7 + 1); /* <workspace_path> + '/' + 'backing' + '\0' */
    meta_file_path    = (char*) sbrk(base_path_length + 1 + 4 + 1); /* <workspace_path> + '/' + 'meta' + '\0' */
