...
//        printf("arena_block_t:\n");
        node = (arena_block_t*) mysbrk(sizeof(arena_block_t));
...
void* arena_allocate(arena_t *arena, uint32_t n_bytes) {
    ...
    if (n_bytes <= SCLASS_SMALL_MAX) {
        ...
        uint32_t real_n_bytes = n_bytes + sizeof(nvm_block);
        n_bytes = real_n_bytes;
...
//        printf("arena_block_t:\n");
        ...
        if (nvm_run->version < current_version) {
            ...
            if (run == nvm_run->vdata) {
                ...
                mysbrkfree(tmp_run,64);  //arena_run_t* = 64
            }
...
//    printf("arena_run_t:\n");
    run = (arena_run_t*) mysbrk(sizeof(arena_run_t));
    run->bin = bin;
    ...
    if (free_block->n_pages > 1) {
        ...
        nvm_run = run->nvm_run;
        mysbrkfree(free_block,48);  //arena_block_t * = 48
...
//    printf("arena_block_t:\n");
    free_block = (arena_block_t*) mysbrk(sizeof(arena_block_t));
...
//    printf("arena_run_t:\n");
    arena_run_t *run = (arena_run_t*) mysbrk(sizeof(arena_run_t));
...
//    printf("arena_block_t:\n");
    arena_block_t *block = (arena_block_t*) mysbrk(sizeof(arena_block_t));
    block->nvm_block = nvm_block;
    block->n_pages = nvm_block->n_pages;
    block->arena = arenas[nvm_block->arena_id];
    return block;
...
void arena_teardown(arena_t *arena) {
    ...
    tree_for_each_entry_safe(node, tmp, arena->free_pageruns, link) {
        ...
        mysbrkfree(node,48);  // arena_block_t * =48
    }
    /* iterate through bins and delete all run headers */
    for (i=0; i<31; ++i) {
        bin = &arena->bins[i];
        if (bin->current_run) {
            mysbrkfree(bin->current_run,64);  // arena_run_t * 64
            bin->current_run = NULL;
        }
        while (bin->runs) {
            run = bin->runs;
            bin->runs = bin->runs->next;
            mysbrkfree(run,64); // arena_run_t * 64
        }
    }
    /* free arena object itself */
    mysbrkfree(arena,2040);  //arena_t  2040
}
