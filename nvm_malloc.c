...
void nvm_free(void *ptr, void **link_ptr1, void *target1, void **link_ptr2, void *target2) {
    ...
    if (rel_ptr % CHUNK_SIZE == sizeof(nvm_huge_header_t)) {
        ...
        printf("huge_t:\n");
        huge = (huge_t*) mysbrk(sizeof(huge_t)); 
...
size_t get_original_size(void * ptr) {
    uintptr_t rel_ptr = __NVM_ABS_TO_REL(ptr);
    if (rel_ptr % CHUNK_SIZE == sizeof(nvm_huge_header_t)) {
        nvm_huge_header_t *nvm_huge = (nvm_huge_header_t*) (ptr - sizeof(nvm_huge_header_t));
        return((4096*1024*(nvm_huge->n_chunks)-sizeof(nvm_huge_header_t)));
    } else {
        ...
        if (GET_USAGE(nvm_block->state) == USAGE_BLOCK) {
            return((4096*(nvm_block->n_pages)-sizeof(nvm_block_header_t)));
        } else {
            /* freeing a small element */
            nvm_run_header_t *nvm_run = (nvm_run_header_t*) nvm_block;
            return(nvm_run->vdata->elem_size);
        }
    }
...
void nvm_initialize_empty() {
    ...
    *(uint64_t*) meta_info = 1;
    ...
    arenas = (arena_t**) sbrk(INITIAL_ARENAS * sizeof(arena_t*));
    for (i=0; i<INITIAL_ARENAS; ++i) {
        arena = (arena_t*) mysbrk(sizeof(arena_t));
                ...
                if (usage == USAGE_FREE) {
                    ...
                    if (nvm_run->version < current_version) {
                        ...
                        if (tmp_run == nvm_run->vdata) {
                            ...
                            mysbrkfree(run,64); // 64 arena_run_t *
                        }
    ...
    for (i=0; i<max_log_entries; ++i) {
        ...
        if (usage == USAGE_HUGE) {
            ...
            if (state == STATE_PREFREE) {
                ...
                PERSIST(nvm_block);
//                printf("arena_block_t:\n");
                block = (arena_block_t*) mysbrk(sizeof(arena_block_t));
...
void nvm_teardown() {
    /* WARNING: this method is NOT thread safe! Make sure all nvm_mysbrk operations
    ...
    mysbrkfree(arenas,160); // arena_t **  size 160
