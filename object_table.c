    ...
    while (1) {
        for (i=0; i<63; ++i) {
            ...
            if (keep) {
                ...
                printf("object_table_entry_t:\n");
                entry = mysbrk(sizeof(object_table_entry_t)); 
...
int ot_insert(const char *id, void *data_ptr) {
    ...
    printf("object_table_entry_t:\n");
    entry = (object_table_entry_t*) mysbrk(sizeof(object_table_entry_t));
...
int ot_remove(const char *id) {
    ...
    if (!chainhash_end(it)) {
        ...
        slot_buffer_n_free += 1;
        mysbrkfree((void*)entry, 80);  // object_table_entry_t*
        return OT_OK;
    } else {
        return OT_FAIL;
    }
...
void ot_teardown() {
    ...
    for (it = chainhash_begin(ot, ot_hashmap); !chainhash_end(it); ) {
        ...
        mysbrkfree(value, 80); //object_table_entry_t*
